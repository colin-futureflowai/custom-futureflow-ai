...
interface BookDetailsProps {
  book: BookData
  isVisible: boolean
  textColor: string // Added textColor prop for dynamic text color transitions
}
...
export function BookDetails({ book, isVisible, textColor }: BookDetailsProps) {
  ...
  return (
    <div
      ...
      style={{
        transition: "opacity 1000ms ease-out, color 700ms ease-out", // Added color transition
      }}
    >
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" size="lg" className={`flex-1 transition-colors duration-700 ${buttonClass}`} asChild>
          <a href="https://v0.dev" target="_blank" rel="noopener noreferrer">
            <ShoppingCart className="w-5 h-5 mr-2" />
            Add to Cart
          </a>
        </Button>
        <Button variant="outline" size="lg" className={`transition-colors duration-700 ${buttonClass}`} asChild>
          <a href="https://v0.dev" target="_blank" rel="noopener noreferrer">
      ...
      <div className="grid grid-cols-2 gap-4 pt-4">
        ...
        <div className="text-center p-4">
          ...
          />
          <div className={`text-sm transition-colors duration-700 ${lightTextClass}`}>Available</div>
          <div className={`text-xs transition-colors duration-700 ${veryLightTextClass}`}>In stock</div>
        </div>
      </div>
    </div>
  )
